...
The owner authored two company capability decks during the Legend Technologies years (2013–2018):
...
years of the career, and the owner wants them kept in the repository "inside an appropriately named
...
2013–2018 claims (a slide names the owner as the slip-ring division's designer; an org chart names
them ENTI's technical manager). And the decks contain third-party personal data — directors' names,
...
A fourth kind of source appeared while planning: the owner's own account of owning a mechanical
...
in no record but the owner's words. ADR-0008 says a claim with no public referent is cut. Cutting the
most formative experience of the era because nobody else wrote it down would be honesty by amputation.

## Decision

1. A top-level `source/` tree holds **owner-authored source material**: `source/decks/<deck-id>/` (a
   ...
   v15 PDF's provenance), and `source/owner-notes/` (dated, first-person statements by the owner,
   verbatim with light punctuation, each headed `Source: owner, <date>`).
2. **Redaction is mechanical and recorded.** The extractor (`scripts/deck-extract.py`) applies a
   ...
   every file ≤ 2 MiB, every owner note dated in name and header.
...
4. **Citation rule.** A site claim drawn from source material names its origin in the content-file
   comment (deck + slide, or the owner note's date). A claim resting **only** on an owner statement is
   rendered with a visible _self-reported_ marker; a claim corroborated by a third-party record (a
   recommendation, a regulator, a client's site) carries none. Owner statements become citable without
...
- The _self-reported_ marker is new UI surface and a new content invariant (a tradeoff text that
  contains the marker must cite an owner note). It costs a few words per claim; it buys the right to
  put shop-floor experience on the page at all.
...
- **Git LFS for the originals.** GitHub Pages does not serve LFS objects, every clone pulls ~290 MB,
  and it needs tooling that is not installed; the owner chose release assets when offered both.
...
- **Writing the owner's account into the pages as plain fact.** That is the exact drift ADR-0008
