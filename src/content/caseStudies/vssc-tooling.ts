...
export const vsscTooling: CaseStudy = {
  ...
  problem: [
    ...
    'The hardware here sits on PSLV’s strap-on boosters. A PSOM is one of the six solid strap-on motors clustered around the first stage, and two of the six also carry SITVC — the core stage steers in pitch and yaw this way, and the pair of strap-on units augments roll control, which is exactly why the cones come in SITVC and non-SITVC variants that must never be confused — — Secondary Injection Thrust Vector Control, which steers the vehicle by injecting an aqueous strontium-perchlorate solution into the nozzle exhaust rather than gimbaling the nozzle, using tanks strapped to the motor and pressurised with nitrogen. That split is why the line runs SITVC and non-SITVC nose-cone variants, plus the SITVC aft shroud around the injection end.',
